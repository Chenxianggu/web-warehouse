...
Production releases are built by GitHub Actions, stored in GitHub Container Registry (GHCR), and deployed over SSH when a semantic version tag such as `v1.2.3` is pushed.
...
sudo chown deploy:deploy /opt/warehouse
```
...
IMAGE_REPOSITORY=ghcr.io/chenxianggu/web-warehouse
```

For a private GHCR package, sign in once on the server with a token that only has `read:packages` access:

```bash
echo "$GHCR_READ_TOKEN" | docker login ghcr.io --username Chenxianggu --password-stdin
```
...
- `PROD_KNOWN_HOSTS`: output of `ssh-keyscan -H -p <port> <host>` verified against the server host key
...
Open the `Deploy production` workflow in GitHub Actions, choose **Run workflow**, and provide a previously published tag such as `v1.0.0`. The workflow deploys that immutable image without rebuilding it.
